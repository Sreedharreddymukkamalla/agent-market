...
export default function MCPSecurityScanner({ serverUrl = "", token = "" }: { serverUrl?: string; token?: string; }) {
  ...
  const scoreColor =
    clampedScore === null ? "#6b7280"
    : clampedScore >= 80  ? "#22c55e"
    : clampedScore >= 50  ? "#eab308"
    : clampedScore >= 25  ? "#f97316"
    : "#ef4444";

  const scoreLabel =
    clampedScore === null ? "—"
    : clampedScore >= 80  ? "SAFE"
    : clampedScore >= 50  ? "CAUTION"
    : clampedScore >= 25  ? "RISKY"
    : "DANGEROUS";
    ...
    <div style={S.root}>
      <div style={S.header}>
        <div style={S.headerLeft}>
          ...
          <div>
            ...
            <div style={S.subtitle}>Checks for tool poisoning, prompt injection & misconfigurations</div>
          </div>
        </div>
        {clampedScore !== null && (
          <div style={S.scoreBox}>
            <div style={{ ...S.scoreBadge, color: scoreColor, borderColor: scoreColor + "44" }}>
      ...
      {scanning && (
        <div style={S.progressWrap}>
          ...
          <div style={S.progressLabel}>Connecting and analysing server metadata…</div>
        </div>
      )}
        ...
        <div style={S.results}>
          <div style={S.summary}>
            { ["CRITICAL","HIGH","MEDIUM","LOW","PASS"].map((sev) => {
              const count = results.findings.filter((f: any) => f.severity === sev).length;
              if (count === 0) return null;
              const s = SEVERITY[sev as keyof typeof SEVERITY];
              return (
                <div key={sev} style={{ ...S.summaryChip, background: s.bg, borderColor: s.color + "44" }}>
            ...
            <div style={S.summaryMeta}>
              {results.toolCount} tools · {results.resourceCount} resources · {results.promptCount} prompts scanned
            </div>
          </div>
              ...
              return (
                <div key={i} style={{ ...S.finding, background: s.bg, borderColor: s.color + "33" }}>
                  <div style={S.findingHeader}>
                    <span style={{ ...S.sevBadge, color: s.color, borderColor: s.color + "55" }}>
                      {f.severity}
                    </span>
                  ...
                  {f.evidence && (
                    <pre style={{ ...S.evidence, borderColor: s.color + "22" }}>{f.evidence}</pre>
                  )}
                  {f.recommendation && (
          ...
          <div style={S.footer}>
            Scanned at {new Date(results.scannedAt).toLocaleString()} · This scan analyses static metadata only. Runtime behaviour may differ.
          </div>
        </div>
      )}
    </div>
...
const C = {
  bg: "#0d0d0f", surface: "#141416", border: "#1e1e22",
  text: "#e8e8f0", muted: "#5a5a6e", accent: "#7c6af7",
...
const S: any = {
  ...
  scoreLabel: { fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", marginTop: 2 },
  inputRow: {
  ...
  errorBanner: {
    background: "#2a0f0f", borderTop: `1px solid ${C.red}44`,
    color: C.red, padding: "8px 20px", fontSize: 12,
  },
  progressWrap: { padding: "16px 20px" },
  progressBar: {
    height: 3, background: "#1e1e22", borderRadius: 2, overflow: "hidden",
  },
  progressFill: {
    height: "100%", width: "60%", background: "#7c6af7",
    borderRadius: 2,
  ...
  finding: {
    border: "1px solid", borderRadius: 8, padding: 14,
    display: "flex", flexDirection: "column", gap: 8,
  },
  findingHeader: { display: "flex", alignItems: "center", gap: 8 },
  sevBadge: {
    border: "1px solid", borderRadius: 4, padding: "1px 7px",
  ...
  evidence: {
    background: "#08080c", border: "1px solid",
    borderRadius: 5, padding: "8px 12px",
    fontSize: 11, color: "#f97316", margin: 0,
    fontFamily: "inherit", overflowX: "auto", maxHeight: 120,
