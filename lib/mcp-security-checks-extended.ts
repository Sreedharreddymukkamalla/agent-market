...
export async function checkRugPull(
  url: string,
  ...
  try {
    ...
    const hashes1 = Object.fromEntries(
      (tools1 || []).map((t: any) => [t.name, hashToolDefinition(t)]),
    );
    ...
    const hashes2 = Object.fromEntries(
      (tools2 || []).map((t: any) => [t.name, hashToolDefinition(t)]),
    );
    ...
    if (changed.length > 0 || added.length > 0 || removed.length > 0) {
      ...
      findings.push({
        check: "RUG_PULL",
        severity: "PASS",
        title: "Tool definitions stable across two fetches",
        description: `${tools1?.length ?? 0} tool(s) re-verified — no changes detected within this scan.`,
      });
    }
  } catch (e: any) {
    findings.push({
      check: "RUG_PULL",
      severity: "INFO",
      title: "Rug pull check could not complete",
...
export function checkCrossOriginShadowing(tools: any[], findings: any[]) {
  const shadowedNames = tools
    .map((t) => t.name)
    .filter((name) => TRUSTED_TOOL_NAMES_TO_SHADOW.includes(name));
...
export async function checkSensitiveDataInResources(
  url: string,
  token: string | undefined,
  resources: any[],
  findings: any[],
  ...
  try {
    ...
    for (const resource of sample) {
      try {
        ...
        const content =
          result.contents?.map((c: any) => c.text || c.blob || "").join(" ") ??
          "";
  ...
  if (secretHits.length > 0) {
    ...
    findings.push({
      check: "SENSITIVE_DATA",
      severity: "PASS",
      title: "No obvious secrets detected in sampled resources",
      description: `Sampled ${Math.min(5, resources.length)} of ${resources.length} resource(s) — no known secret patterns found.`,
    });
  }
}
...
export function checkSchemaStrictness(tools: any[], findings: any[]) {
  ...
  if (laxTools.length > 0) {
    findings.push({
      ...
      evidence:
        laxTools.slice(0, 10).join(", ") +
        (laxTools.length > 10 ? ` +${laxTools.length - 10} more` : ""),
      recommendation:
    ...
    findings.push({
      check: "SCHEMA_STRICT",
      severity: "PASS",
      title: "All tool schemas use strict additionalProperties:false",
      description: "Tool input schemas correctly reject unexpected parameters.",
    });
  }
}
...
export function checkPromptInjection(prompts: any[], findings: any[]) {
  ...
  if (poisoned.length > 0) {
    ...
    findings.push({
      check: "PROMPT_INJECT",
      severity: "PASS",
      title: "No injection patterns in prompt templates",
      description: `Scanned ${prompts.length} prompt template(s) — clean.`,
    });
  }
}
...
export function checkDescriptionEntropy(tools: any[], findings: any[]) {
  ...
  for (const tool of tools) {
    ...
    if (entropy > 5.5) {
      suspicious.push(
        `"${tool.name}" entropy=${entropy.toFixed(2)} (desc length: ${desc.length})`,
      );
    }
  }
...
export function checkUrlParams(tools: any[], findings: any[]) {
  ...
  for (const tool of tools) {
    ...
    for (const paramName of Object.keys(props)) {
      if (URL_PARAM_NAMES.includes(paramName.toLowerCase()))
        urlParams.push(`${tool.name}.${paramName}`);
    }
  }
...
export function checkVersionLeakage(serverInfo: any, findings: any[]) {
  ...
  const leaked = stackIndicators.filter(
    (s) => name.toLowerCase().includes(s) || version.toLowerCase().includes(s),
  );
  ...
  if (semverMatch) {
    ...
    if (major === 0) {
      findings.push({
        ...
        recommendation:
          "Prefer stable, production-grade server versions for use in agent workflows.",
      });
    }
  }
}
  ...
  try {
    ...
    if (acao === "*") {
      ...
      findings.push({
        ...
        recommendation:
          "Use a strict allowlist of permitted origins. Never reflect the incoming Origin header.",
      });
      ...
      findings.push({
        ...
        recommendation:
          "Ensure the allowed origin is a specific, trusted domain — not dynamic.",
      });
    } else if (!acao) {
      findings.push({
        check: "CORS",
        ...
        description:
          "Server did not return a wildcard or reflective CORS header.",
      });
    }
  } catch {
    findings.push({
      check: "CORS",
      severity: "INFO",
      title: "CORS preflight check skipped",
      description:
        "OPTIONS request timed out or was not supported by the server.",
    });
  }
}
